...
const getAuthByIdModel = async (id_user) => {
	return new Promise((resolve,reject)=>
		Pool.query(`SELECT * FROM user_auth WHERE id_user='${id_user}'`,(err,res)=>{
			if(!err){
				return resolve(res)
			} else {
				reject(err)
...
const createAuthModel = async(data) => {
    let  {id_user, email, password, name, phone,  role, position, verifyotp} = data
    console.log(data)
    return new Promise((resolve,reject) => 
        Pool.query(`INSERT INTO user_auth (id_user, email, password, name, phone, position, role, verifyotp, created_at, updated_at) VALUES ('${id_user}','${email}', '${password}', '${name}', '${phone}', '${position}', '${role}', '${verifyotp}', NOW(), NULL)`,(err,res) => 
        {
            if(!err){
				return resolve(res)
			} else {
...
const activatedUser = async(id_user) => {
	...
	return new Promise((resolve,reject)=>
		Pool.query(`UPDATE user_auth SET isverify=true WHERE id_user='${id_user}'`,(err,res)=>{
			if(!err){
				return resolve(res)
			} else {
