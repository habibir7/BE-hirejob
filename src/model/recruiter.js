...
const getRecuiterByIdModel = async (id_user) => {
  return new Promise((resolve, reject) => {
    Pool.query(
      `SELECT user_auth.name, user_auth.position, user_auth.email, detail_profile_recruiter.* FROM detail_profile_recruiter JOIN user_auth ON detail_profile_recruiter.id_user = user_auth.id_user where detail_profile_recruiter.id_user='${id_user}'`,
      (err, res) => {
        if (!err) {
          return resolve(res);
        } else {
